...
const (
	...
	// the three-way patch
	clusterRouterLastNodeAppliedNodeStatus = "cluster-router.io/last-applied-node-status"
	clusterRouterLastNodeAppliedObjectMeta = "cluster-router.io/last-applied-object-meta"
)
...
func NewNodeController(p plugins.NodeProvider, node *corev1.Node, nodes v1.NodeInterface, opts ...NodeControllerOpt) (*NodeController, error) {
	n := &NodeController{
		p:          p,
		serverNode: node,
		nodes:      nodes,
...
func prepareThreewayPatchBytesForNodeStatus(nodeFromProvider, apiServerNode *corev1.Node) ([]byte, error) {
	...
	*/
	...
	oldVKStatus, ok1 := apiServerNode.Annotations[clusterRouterLastNodeAppliedNodeStatus]
	oldVKObjectMeta, ok2 := apiServerNode.Annotations[clusterRouterLastNodeAppliedObjectMeta]
	...
	if ok1 && ok2 {
		...
		if err != nil {
			return nil, pkgerrors.Wrapf(err, "Cannot unmarshal old node object metadata (key: %q): %q", clusterRouterLastNodeAppliedObjectMeta, oldVKObjectMeta)
		}
		err = json.Unmarshal([]byte(oldVKStatus), &oldNode.Status)
		if err != nil {
			return nil, pkgerrors.Wrapf(err, "Cannot unmarshal old node status (key: %q): %q", clusterRouterLastNodeAppliedNodeStatus, oldVKStatus)
		}
	}
	...
	clusterRouterLastNodeAppliedObjectMetaBytes, err := json.Marshal(newNode.ObjectMeta)
	if err != nil {
		return nil, pkgerrors.Wrap(err, "Cannot marshal object meta from provider")
	}
	newNode.Annotations[clusterRouterLastNodeAppliedObjectMeta] = string(clusterRouterLastNodeAppliedObjectMetaBytes)

	clusterRouterLastNodeAppliedNodeStatusBytes, err := json.Marshal(newNode.Status)
	if err != nil {
		return nil, pkgerrors.Wrap(err, "Cannot marshal node status from provider")
	}
	newNode.Annotations[clusterRouterLastNodeAppliedNodeStatus] = string(clusterRouterLastNodeAppliedNodeStatusBytes)
	...
	if objectMetaWithLabelsAndAnnotations != nil {
		...
		if objectMetaWithLabelsAndAnnotations.Annotations != nil {
			...
			for key := range objectMetaWithLabelsAndAnnotations.Annotations {
				if key == clusterRouterLastNodeAppliedNodeStatus || key == clusterRouterLastNodeAppliedObjectMeta {
					continue
				}
